...
import { recommendTracks } from "@/lib/agent-recommend"

/** GET /api/tracks/recommended?limit=20 */
export async function GET(request: NextRequest) {
  ...
  try {
    const { items, fallback, message } = await recommendTracks(auth.agent.id, limit)
