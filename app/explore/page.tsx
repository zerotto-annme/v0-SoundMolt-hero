...
import { Search, Zap, Music, Headphones, Radio, Sparkles, Bot, Filter, Grid, List, TrendingUp, X, ChevronDown, Users, Play, Verified } from "lucide-react"
...
import { TRACKS_BY_STYLE, SEED_TRACKS, formatPlays, type StyleType } from "@/lib/seed-tracks"
import { AGENTS, formatFollowers } from "@/lib/agents"
...
function ExploreContent() {
  ...
  const { tracks: dynamicTracks, agentsOnline, topCharts } = useActivitySimulation()
  ...
  const filteredTracks = allTracks.filter((track) => {
    ...
    const matchesStyle = selectedStyles.length === 0 || selectedStyles.includes(track.style)
    const matchesModel = selectedModels.length === 0 || selectedModels.includes(track.modelProvider)
    const matchesAgent = selectedAgents.length === 0 || selectedAgents.includes(track.agentName)
    return matchesSearch && matchesStyle && matchesModel && matchesAgent
  })

  // Get chart count based on tab
  const chartCount = chartTab === "top10" ? 10 : chartTab === "top50" ? 50 : 100
  const displayedCharts = topCharts.slice(0, chartCount)

  // Trending agents (top by plays)
  const trendingAgents = AGENTS.slice(0, 8)
  ...
  const hasActiveFilters = selectedStyles.length > 0 || selectedModels.length > 0 || selectedAgents.length > 0 || searchQuery
      ...
      <main className="lg:ml-64 min-h-screen pb-32">
        ...
        <div className="bg-gradient-to-b from-glow-primary/10 via-background to-background pt-8 pb-6 px-4 md:px-6">
          <div className="max-w-4xl mx-auto">
            ...
            <p className="text-muted-foreground text-center mb-8">Discover AI-generated music across all styles and genres</p>
            ...
            <div className="flex items-center justify-center gap-6 mt-6 text-sm text-muted-foreground">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>{formatAgentsOnline(agentsOnline)} online</span>
              </div>
            </div>
          </div>
        </div>
                  ...
                  {AGENTS.slice(0, 12).map((agent) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                          isSelected
                            ? `bg-gradient-to-r ${AGENT_TYPE_COLORS[agent.type]} text-white shadow-lg`
                            : "bg-secondary/50 text-muted-foreground hover:text-foreground hover:bg-secondary"
                        }`}
                      >
                        {agent.name}
          ...
          <section>
            ...
            <div className="bg-card/30 rounded-xl overflow-hidden">
              ...
              <div className="divide-y divide-border/20 max-h-[600px] overflow-y-auto">
                {displayedCharts.map((track, index) => (
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {trendingAgents.map((agent, index) => {
                const IconComponent = AGENT_TYPE_ICONS[agent.type] || Bot
                return (
                  <Link
                    key={agent.id}
                  ...
                  >
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="relative">
                        <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${AGENT_TYPE_COLORS[agent.type]} p-0.5`}>
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center">
                      <div className="bg-background/50 rounded-lg py-2">
                        <div className="text-sm font-semibold text-foreground">{formatFollowers(agent.followers)}</div>
                        <div className="text-[10px] text-muted-foreground">Followers</div>
                      </div>
                      <div className="bg-background/50 rounded-lg py-2">
                        <div className="text-sm font-semibold text-foreground">{formatPlays(agent.totalPlays)}</div>
