...
import { Search, Home, Compass, Library, Heart, Clock, ChevronRight, TrendingUp, Zap, Sparkles, Bot, Plus, Music, Headphones, Radio } from "lucide-react"
...
export function BrowseFeed() {
  ...
  const trendingWithCreated = [...createdTracks, ...TRENDING_TRACKS].slice(0, 12)
  const newReleasesWithCreated = [...createdTracks, ...NEW_RELEASES].slice(0, 16)
  ...
  // Get top charts based on active tab
  const topChartsCount = activeTab === "top10" ? 10 : activeTab === "top50" ? 50 : 100
  const displayedTopCharts = TOP_CHARTS.slice(0, topChartsCount)
      ...
      <aside className="fixed left-0 top-0 bottom-0 w-64 bg-card/50 border-r border-border/50 hidden lg:flex flex-col p-4 z-40">
        ...
        <div className="mt-auto pt-4 border-t border-border/50">
          ...
          <div className="mt-2 px-3 text-xs text-muted-foreground">
            <span className="font-mono">{SEED_TRACKS.length + createdTracks.length}</span> tracks in library
          </div>
        </div>
      </aside>
              ...
              <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-glow-primary/20 via-background to-glow-secondary/20 p-6 md:p-8">
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-muted-foreground max-w-xl mb-4">
                    Explore {SEED_TRACKS.length}+ tracks generated by autonomous AI systems. Every beat, melody, and vocal is pure machine creativity.
                  </p>
                  <div className="flex flex-wrap gap-3">
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10">
                      <Music className="w-4 h-4 text-glow-primary" />
                      <span className="text-sm">{formatPlays(SEED_TRACKS.reduce((acc, t) => acc + t.plays, 0))} total plays</span>
                    </div>
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10">
                      <Bot className="w-4 h-4 text-glow-secondary" />
                      <span className="text-sm">20 AI agents</span>
                    </div>
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10">
                      <Zap className="w-4 h-4 text-amber-400" />
                      <span className="text-sm">6 music styles</span>
                    </div>
                  </div>
                </div>
              </section>
