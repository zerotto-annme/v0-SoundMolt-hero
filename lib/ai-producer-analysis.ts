...
export async function generateProducerReport(
  ...
  const systemPrompt =
    ...
    "Each entry MUST contain these labelled blocks in this EXACT order on their own lines: [N] (an actual numeric step ID written literally as [1], [2], [3] — NEVER output the literal text [STEP ID] / [N] / [number] in the JSON), FIXING, CHANNEL, GO TO, FIND, CLICK, SET, CHECK, STOP WHEN, WARNING, IF NOT AVAILABLE, RESULT. SELECT, INSERT and AUTOMATE are OPTIONAL and may be added (SELECT before CLICK, INSERT between CLICK and SET, AUTOMATE between SET and CHECK) ONLY when they make the instruction more actionable. CHECK LOUDNESS is MANDATORY whenever the entry references LUFS / loudness / integrated loudness / true peak (insert it immediately after CHECK and before STOP WHEN). SIDECHAIN SETUP is MANDATORY whenever the entry references sidechain / side-chain / ducking / kick-bass routing (insert it immediately after CHECK / CHECK LOUDNESS and before STOP WHEN). STOP WHEN and WARNING are MANDATORY in EVERY entry — STOP WHEN names the OBJECTIVE measurable condition that signals the fix is correctly applied (a meter reading, a numeric LUFS / dB / Hz value, a visible spectrum / waveform state); WARNING names the audible failure mode if the user pushes the fix too far AND tells the user how to back off in concrete numbers. NOTHING is allowed after RESULT — no closing summary, no repeated MAIN ISSUE / FIXING line, no extra commentary.\n" +
    ...
    "Conditional SIDECHAIN SETUP block (insert immediately after CHECK / CHECK LOUDNESS and before STOP WHEN whenever the entry mentions sidechain / side-chain / ducking / kick-bass routing — adapt routing wording to the SELECTED DAW; this is the FULL 8-step routing flow, not a one-line note):\n" +
...
Each entry MUST contain these labelled blocks in this EXACT order on their own lines: [N] (an actual numeric step ID written literally as [1], [2], [3] — NEVER output the literal text [STEP ID] / [N] / [number] in the JSON), FIXING, CHANNEL, GO TO, FIND, CLICK, SET, CHECK, STOP WHEN, WARNING, IF NOT AVAILABLE, RESULT. SELECT, INSERT and AUTOMATE are OPTIONAL (SELECT before CLICK, INSERT between CLICK and SET, AUTOMATE between SET and CHECK) and may be added ONLY when they make the instruction more actionable for THIS track. CHECK LOUDNESS is MANDATORY whenever the entry mentions LUFS / loudness / integrated loudness / true peak — insert it immediately after CHECK and before STOP WHEN. SIDECHAIN SETUP is MANDATORY whenever the entry mentions sidechain / side-chain / ducking / kick-bass routing — insert it immediately after CHECK / CHECK LOUDNESS and before STOP WHEN. STOP WHEN and WARNING are MANDATORY in EVERY entry — STOP WHEN names the OBJECTIVE measurable condition that signals the fix is correctly applied (meter reading, numeric LUFS / dB / Hz value, visible spectrum / waveform state); WARNING names the audible failure mode if the user pushes the fix too far AND tells the user how to back off in concrete numbers. NOTHING is allowed after RESULT — no closing summary, no repeated FIXING / MAIN ISSUE line.
...
Conditional SIDECHAIN SETUP block (insert immediately after CHECK / CHECK LOUDNESS and before STOP WHEN whenever the entry mentions sidechain / side-chain / ducking / kick-bass routing — adapt the routing wording to ${dawLb}; this is the FULL 8-step routing flow, not a one-line note):
...
10. FIND, CHECK, STOP WHEN, WARNING and IF NOT AVAILABLE are MANDATORY in EVERY entry — never omit them. CHECK LOUDNESS is MANDATORY whenever LUFS is referenced; SIDECHAIN SETUP (the FULL 8-step routing flow) is MANDATORY whenever sidechain is referenced. STOP WHEN MUST cite a measurable condition (meter / LUFS / dB / Hz / spectrum / waveform reading); WARNING MUST name the audible failure mode AND the exact numeric back-off (e.g. "if bass disappears reduce EQ cut by 1 dB", "if master loses dynamics reduce drive by 1–2 dB").
