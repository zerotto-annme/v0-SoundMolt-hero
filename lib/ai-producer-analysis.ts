...
export async function generateProducerReport(
  ...
  const systemPrompt =
    ...
    "Conditional SIDECHAIN SETUP block (insert immediately after CHECK and before IF NOT AVAILABLE whenever the entry mentions sidechain / side-chain / ducking / kick-bass routing — adapt routing wording to the SELECTED DAW):\n" +
    "SIDECHAIN SETUP:\\n1. Insert a Compressor on the channel that should duck (e.g. Bass channel).\\n2. Enable the Sidechain input on that compressor (Cubase: Side-Chain button top-right of Compressor; Ableton: \\\"Sidechain\\\" toggle on Compressor; FL Studio: route sender via the Mixer arrow; Logic: \\\"Side Chain\\\" menu top-right).\\n3. Route the trigger source (e.g. Kick) into the sidechain input of that compressor.\\n4. Set Threshold so peaks of the trigger pull ~-4 dB of gain reduction; Attack 1–10 ms; Release 80–150 ms; Ratio 4:1.\n" +
    ...
    `${dawExampleFor(dawId)}\n` +
    "RULES (HARD): (1) Every entry MUST mention the SELECTED DAW by name AT LEAST ONCE in either GO TO or CLICK. (2) GO TO and CLICK MUST be NUMBERED LISTS (1. 2. 3. 4.) and CLICK MUST contain AT LEAST 3 numbered steps that describe the full navigation chain (which area to click → which window/browser opens → which path to follow → which plugin to load). (3) SET MUST end with a colon and each (param: value) pair MUST be on its own indented line; add a (range: …) note next to any param where a safe musical range exists. (4) NO generic placeholders like \"the mixer\" or \"insert a limiter\" — use the DAW vocabulary from the user prompt verbatim. (5) Do NOT invent plugins; pick from the SELECTED DAW's stock plugin list given in the DAW VOCABULARY block, or say \"a generic <type>\" if the DAW is unknown. (6) Each entry MUST be tied to a specific recommendation from the report — do NOT add generic mastering homework that is unrelated to THIS track. (7) NO \" | \" separators, NO single-line inlining. (8) FIND, CHECK and IF NOT AVAILABLE are MANDATORY in EVERY entry — never omit them; CHECK LOUDNESS is MANDATORY whenever LUFS is referenced; SIDECHAIN SETUP is MANDATORY whenever sidechain is referenced. (9) CHECK MUST include a troubleshooting step that tells the user how to back off (reduce gain / drive / amount by 1–2 dB) if the effect is too strong.\n\n" +
...
Conditional SIDECHAIN SETUP block (insert immediately after CHECK and before IF NOT AVAILABLE whenever the entry mentions sidechain / side-chain / ducking / kick-bass routing — adapt the routing wording to ${dawLb}):
SIDECHAIN SETUP:\\n1. Insert a Compressor on the channel that should duck (e.g. Bass channel).\\n2. Enable the Sidechain input on that compressor (use ${dawLb}'s exact UI label from DAW VOCABULARY).\\n3. Route the trigger source (e.g. Kick) into the sidechain input of that compressor.\\n4. Set Threshold so peaks of the trigger pull ~-4 dB of gain reduction; Attack 1–10 ms; Release 80–150 ms; Ratio 4:1.
...
2. GO TO and CLICK MUST be NUMBERED LISTS using the format "1. ", "2. ", "3. ", "4. " on their own lines, and CLICK MUST contain AT LEAST 3 numbered steps that describe the full navigation chain (which area to click → which window/browser opens → which path to follow → which plugin to load).
...
8. FIND, CHECK and IF NOT AVAILABLE are MANDATORY in EVERY entry — never omit them. CHECK LOUDNESS is MANDATORY whenever LUFS is referenced; SIDECHAIN SETUP is MANDATORY whenever sidechain is referenced.
9. CHECK MUST include a troubleshooting step that tells the user how to back off (reduce gain / drive / amount by 1–2 dB) if the effect is too strong.
