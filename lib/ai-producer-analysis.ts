...
export async function generateProducerReport(
  ...
  const systemPrompt =
    ...
    `${dawExampleFor(dawId)}\n` +
    "RULES (HARD): (1) Every entry MUST start with [<step number>] on its OWN line and the next line MUST be FIXING: <problem> mirroring a FIX from the report — no entry without both. (2) Every entry MUST mention the SELECTED DAW by name AT LEAST ONCE in either GO TO or CLICK. (3) GO TO and CLICK MUST be NUMBERED LISTS (1. 2. 3. 4.) and CLICK MUST contain AT LEAST 3 numbered steps that describe the full navigation chain (which area to click → which window/browser opens → which path to follow → which plugin to load). (4) SET MUST end with a colon and each (param: value) pair MUST be on its own indented line; add a (range: …) note next to any param where a safe musical range exists. (5) CHECK MUST be OBJECTIVE — every check step MUST cite a meter, spectrum analyzer, waveform, gain-reduction meter, LUFS reading, or numeric value the user can SEE. The phrases \\\"listen for\\\", \\\"check the warmth\\\", \\\"check clarity\\\", \\\"check the vibe\\\" are FORBIDDEN. Replace any subjective listening cue with a measurable instrument-based observation. (6) NO generic placeholders like \\\"the mixer\\\" or \\\"insert a limiter\\\" — use the DAW vocabulary from the user prompt verbatim. (7) Do NOT invent plugins; pick from the SELECTED DAW's stock plugin list given in the DAW VOCABULARY block, or say \\\"a generic <type>\\\" if the DAW is unknown. (8) Each entry MUST be tied to a specific high-impact recommendation from the report; non-critical fixes are SKIPPED, not padded. (9) NO \\\" | \\\" separators, NO single-line inlining. (10) FIND, CHECK and IF NOT AVAILABLE are MANDATORY in EVERY entry — never omit them; CHECK LOUDNESS is MANDATORY whenever LUFS is referenced; SIDECHAIN SETUP (the FULL 7-step routing flow) is MANDATORY whenever sidechain is referenced. (11) CHECK MUST include a troubleshooting step that tells the user how to back off (reduce gain / drive / amount by 1–2 dB) if the effect is too strong. (12) NOTHING after RESULT — no closing line, no repeated FIXING / MAIN ISSUE summary, no extra advice. The entry ENDS at RESULT. (13) Generate AT MOST 6 entries — quality over quantity; if only 3 fixes are truly high-impact for THIS track, return 3.\n\n" +
    ...
    "ANTI-REPETITION (Stage 11 step 6 + FINAL-QUALITY-UPGRADE) — each concrete diagnosed problem may appear in ONLY ONE place across the whole report. Pick the most relevant home (summary OR mix OR mastering OR arrangement OR sound_design OR commercial_potential) and keep the strongest, most actionable wording there. Do NOT restate the same issue verbatim in summary, mix, AND mastering — pick the strongest version, mention the issue only once, and let other sections reference different aspects. NO summary-like recap lines: do NOT add a \"MAIN ISSUE — ...\", \"FIXING: ...\" or any closing recap line AFTER the RESULT block of section.text or AFTER the RESULT line of any daw_instructions entry. section.text contains ONLY the FIVE labelled blocks (MAIN ISSUE / WHY / IMPACT / FIX / RESULT) in that exact order — nothing before, nothing after. daw_instructions entries contain ONLY the labelled blocks defined in DAW MODE — nothing after RESULT.\n\n" +
...
Each entry MUST contain these labelled blocks in this EXACT order on their own lines: [N] (an actual numeric step ID written literally as [1], [2], [3] — NEVER output the literal text [STEP ID] / [N] / [number] in the JSON), FIXING, CHANNEL, GO TO, FIND, CLICK, SET, CHECK, IF NOT AVAILABLE, RESULT. SELECT, INSERT and AUTOMATE are OPTIONAL (SELECT before CLICK, INSERT between CLICK and SET, AUTOMATE between SET and CHECK) and may be added ONLY when they make the instruction more actionable for THIS track. CHECK LOUDNESS is MANDATORY whenever the entry mentions LUFS / loudness / integrated loudness / true peak — insert it immediately after CHECK. SIDECHAIN SETUP is MANDATORY whenever the entry mentions sidechain / side-chain / ducking / kick-bass routing — insert it immediately after CHECK and before IF NOT AVAILABLE. NOTHING is allowed after RESULT — no closing summary, no repeated FIXING / MAIN ISSUE line.
...
Conditional CHECK LOUDNESS block (insert immediately after CHECK whenever the entry references LUFS / loudness / integrated loudness / true peak — adapt the meter name to ${dawLb} using STOCK plugins ONLY from DAW VOCABULARY: Cubase → SuperVision (Loudness module), Ableton → built-in Master level meter + Utility (peak/RMS workflow), FL Studio → Wave Candy (Loudness mode), Logic → Loudness Meter / MultiMeter):
CHECK LOUDNESS:\\n1. Insert the ${dawLb} stock loudness / level meter on the master / target channel after the limiter; if a dedicated LUFS module is unavailable in ${dawLb}, use ${dawLb}'s built-in master peak / RMS meter as the reference.\\n2. Add / enable the Loudness (LUFS) module if the meter is multi-mode.\\n3. Play the FULL track from start to end without stopping.\\n4. Read \"Integrated LUFS\" (or the peak / RMS reading when a LUFS module is unavailable) and adjust the limiter / gain stage until it matches the genre target from GENRE BRAIN.
Conditional SIDECHAIN SETUP block (insert immediately after CHECK and before IF NOT AVAILABLE whenever the entry mentions sidechain / side-chain / ducking / kick-bass routing — adapt the routing wording to ${dawLb}; this is the FULL routing flow, not a one-line note):
...
4. SET MUST end with a colon, each (param: value) pair on its own indented line, with a (range: …) annotation when a safe musical range exists.
5. CHECK MUST be OBJECTIVE — every check step MUST cite a meter, spectrum analyzer, waveform, gain-reduction meter, LUFS reading, or numeric value the user can SEE. The phrases "listen for", "check the warmth", "check clarity", "check the vibe" are FORBIDDEN. Replace any subjective listening cue with a measurable instrument-based observation.
...
10. FIND, CHECK and IF NOT AVAILABLE are MANDATORY in EVERY entry — never omit them. CHECK LOUDNESS is MANDATORY whenever LUFS is referenced; SIDECHAIN SETUP (the FULL 7-step routing flow) is MANDATORY whenever sidechain is referenced.
11. CHECK MUST include a troubleshooting step that tells the user how to back off (reduce gain / drive / amount by 1–2 dB) if the effect is too strong.
